...
export function parseImageDataUrl(url: string): { mediaType: string; data: string } | undefined {
 ...
 */
export async function toolResultBlocksFromText(
  attachments: AttachmentsSaveFace | undefined,
  text: string,
  path?: string,
  ...
  for (const item of parsed) {
    ...
    if (record['type'] === 'text' && typeof record['text'] === 'string') {
      if (record['text'].length > 0) blocks.push({ type: 'text', text: record['text'] })
      continue
    }
    if (record['type'] !== 'image_url') return undefined // 未知块:整体回退,不猜
...
function argsRecord(argsJson: unknown): Record<string, unknown> | undefined {
  if (typeof argsJson === 'object' && argsJson !== null) return argsJson as Record<string, unknown>
  if (typeof argsJson !== 'string') return undefined
  try {
 ...
 */
export function imageReadAlias(
  name: string,
  argsJson: unknown,
): { name: 'read_image'; path: string } | undefined {
  if (name.trim().toLowerCase() !== 'read') return undefined
  const args = argsRecord(argsJson)
  const path = args?.['file_path'] ?? args?.['filePath'] ?? args?.['path']
  if (typeof path !== 'string' || !IMAGE_PATH.test(path.trim())) return undefined
