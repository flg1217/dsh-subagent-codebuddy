  ...
  private async *streamOnce(
    ...
    try {
      ...
      const landToolCall = (
        ...
        if (/"run_in_background"\s*:\s*true|"background"\s*:\s*true/.test(args)) backgroundLaunched = true
        session!.append('assistant/message', {
          turn,
          step,
          message: createAssistantMessage({
      ...
      const handleUpdate = async (update: AcpUpdate): Promise<void> => {
        ...
        if (update.sessionUpdate === 'session_info_update') {
          ...
          if (typeof phase === 'string') agentPhaseSeen = true
      ...
      try {
         ...
         */
        const pollInsertions = (): void => {
          if (!direct || dshSessionId === undefined || session === undefined
            || options.signal?.aborted === true || Date.now() - lastSteerPoll < steerPollMs) return
        ...
        let tailDeadline: number | undefined
        /**
         * 尾巴窗口资格:direct(已打开 step)+ 干净 settle + **CLI 支持
         * agentPhase 心跳**(能力的自然探针)。没有该心跳的老 CLI 无从判断
         * 空闲,退回旧行为——立即收尾,不加延迟。
         */
        const tailEligible = (): boolean => direct && tailQuietMs > 0 && tailCapMs > 0 && agentPhaseSeen
          ...
          try {
            for (;;) {
              if (sessionEnded || stallTimedOut || promptError !== undefined
                || exitError !== undefined || options.signal?.aborted) break
              while (queue.length > 0) {
                const update = queue.shift()
                if (update !== undefined) await handleUpdate(update)
              }
              ...
              if (inFlight > 0) { tailActive = false; continue pump }
              ...
              if (Date.now() - lastUpdateAt >= budgetMs) break
              if (Date.now() >= tailDeadline) break
              await Promise.race([waitForUpdate(), new Promise<void>(resolve => setTimeout(resolve, 200))])
            }
          } finally {
            tailActive = false
          }
