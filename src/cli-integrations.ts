...
export interface IntegrationSyncResult {
  ...
  mcp: { servers: string[]; targetFile: string }
  /** 本次新接入的 skill 名与目标目录。 */
  skills: { linked: string[]; targetDir: string }
}
...
function parseYamlValue(raw: string): string | string[] | undefined {
 ...
 */
export function parseMcpClientRows(text: string): McpRow[] {
  const rows: McpRow[] = []
...
function toCliMcpServer(row: McpRow): Record<string, unknown> | undefined {
...
function readProfileConfigs(profilesRoot: string): string[] {
  const texts: string[] = []
  let entries: string[]
  try {
 ...
 */
export function syncMcpToCodebuddy(profilesRoot?: string, targetFile?: string): { servers: string[]; targetFile: string } {
  ...
  for (const text of readProfileConfigs(root)) {
    for (const row of parseMcpClientRows(text)) {
      ...
      if (server !== undefined) servers[row.serverName] = server
    }
  }
  const names = Object.keys(servers)
  if (names.length === 0) return { servers: [], targetFile: target }
 ...
 */
export function syncSkillsToCodebuddy(options?: SkillSyncOptions): { linked: string[]; targetDir: string } {
...
function skillRoots(options: SkillSyncOptions): string[] {
 ...
 * @returns 各项同步结果。
 */
export function syncCliIntegrations(paths?: CliIntegrationPaths): IntegrationSyncResult {
  let mcp: IntegrationSyncResult['mcp'] = { servers: [], targetFile: '' }
  let skills: IntegrationSyncResult['skills'] = { linked: [], targetDir: '' }
  try {
