...
export function todoToolKind(name: string): 'todowrite' | 'taskcreate' | 'taskupdate' | undefined {
  const key = normalized(name)
  return key === 'todowrite' || key === 'taskcreate' || key === 'taskupdate' ? key : undefined
}

/** CodeBuddy 状态 → dsh 三态;未知按 pending。 */
function normalizeStatus(value: unknown): TodoItem['status'] | 'deleted' {
  if (value === 'completed' || value === 'done') return 'completed'
  if (value === 'in_progress' || value === 'in-progress' || value === 'running') return 'in_progress'
  if (value === 'deleted' || value === 'cancelled' || value === 'canceled' || value === 'removed') return 'deleted'
  return 'pending'
}
...
export class TodoListState {
   ...
   */
  private readonly pendingUpdates = new Map<string, { taskId?: string; status: TodoItem['status'] | 'deleted' }>()

  /** 从会话已提交事件播种(折叠最新一条 todo/write,支持进程内跨轮复用)。 */
  seed(events: readonly { type: string; data?: unknown }[]): void {
    for (const event of events) {
      if (event.type !== 'todo/write') continue
      const todos = (event.data as { todos?: unknown } | undefined)?.todos
      if (Array.isArray(todos)) this.list = this.sanitize(todos)
  ...
  applyToolCall(name: string, args: Record<string, unknown>): boolean {
    switch (normalized(name)) {
      case 'todowrite': {
        if (!Array.isArray(args['todos'])) return false
  ...
  resolveTaskUpdate(callId: string, resultText: string): boolean {
    ...
    if (!/updated\s+task/i.test(resultText)) return false // not found / 报错:不同步
    return this.applyTaskUpdate(intent.taskId, intent.status)
  }

  /** 按 id 改状态(deleted 移除并重排绑定)。 */
  private applyTaskUpdate(id: string | undefined, status: TodoItem['status'] | 'deleted'): boolean {
    ...
    if (index === undefined && id !== undefined) {
      ...
      index = unbound
      if (index !== undefined) this.idIndex.set(id, index)
    }
    if (index === undefined || index >= this.list.length) return false
    if (status === 'deleted') {
      this.list = this.list.filter((_, i) => i !== index)
      this.idIndex.delete(id!)
      // 重排后续绑定
      for (const [key, value] of [...this.idIndex]) {
        if (value > index) this.idIndex.set(key, value - 1)
      }
    } else {
  ...
  applyToolResult(name: string, text: string): void {
    if (normalized(name) !== 'taskcreate') return
  ...
  private sanitize(todos: readonly unknown[]): TodoItem[] {
