 ...
 *
 * 与 spawn 的 --disallowedTools(acp.ts CLI_DISALLOWED_TOOLS)配套:告诉模型
 * 哪些原生工具被禁、对等的 dsh_* 名字、Read 为何保留。放在 system 位置
 * (权威)而不是只靠每条 prompt 尾注——CLI 内置提示词在 user 位置的指令
 * 面前占优,实测尾注拦不住模型直调原生 Bash/Edit。
 */
const TOOL_ROUTING_NOTE = [
  '[工具路由(本环境强制)]',
  'CLI 原生的 Bash/PowerShell/Edit/Write/NotebookEdit/Glob/Grep 已被禁用:对等能力一律用 dsh 前缀版本(dsh_bash、dsh_pwsh、dsh_edit、dsh_write、dsh_glob、dsh_grep)——它们在 dsh 侧执行,受审批/沙箱约束,进会话日志与后台任务面板;被禁工具的调用会被拒绝,不要换别的方式绕过。Read 保留:读图片必须用 Read(结果以图片卡片呈现给用户)。',
].join('\n')
 ...
 */
export function systemInstructionsText(system: string, updated = false): string {
