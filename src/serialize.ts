...
const TOOL_ROUTING_NOTE = [
  '[工具路由(本环境强制)]',
  'CLI 原生的 Bash/PowerShell/Edit/Write/NotebookEdit/Glob/Grep 不可用(工具白名单已移除):对等能力一律用 dsh 前缀版本(dsh_bash、dsh_pwsh、dsh_edit、dsh_write、dsh_glob、dsh_grep)——它们在 dsh 侧执行,受审批/沙箱约束,进会话日志与后台任务面板;被移除工具的调用会失败,不要换别的方式绕过。Read 保留:读图片必须用 Read(结果以图片卡片呈现给用户)。',
].join('\n')
