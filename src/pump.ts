...
export class TurnPump {
  ...
  private steerBlockedLogged = false
  ...
  dispose(): void {
    ...
    // 重试队列随之作废:泵没了就没有重试者。未投出的消息仍未 markForwarded,
    // 下回合的 delta 补发会带上(与旧行为一致:不假装投过)。
  ...
  private onSessionEvent(
    ...
    // 插话/注入的**探测**走事件通道(见 pendingSteers 的注释):不依赖
    // ownEvents() 视图与任何水位线。
    if (event?.type === 'user/message') {
      const data = event.data as { id?: unknown; content?: unknown } | undefined
      if (typeof data?.id === 'string' && data.id.length > 0) this.queueSteer(data.id, textOfContent(data.content))
      return
    }
    if (event?.type === 'agent/inbox/spliced') {
      // 悬挂队列(尚未 claim)也即时投一次:claim 发生在 step 边界,等 claim
      // 再投就晚了一个边界。折叠只在 splice 事件到达时做(罕见),不做轮询。
      for (const insertion of foldPendingInsertions(this.deps.session?.ownEvents?.() ?? [])) {
        if (insertion.steer) this.queueSteer(insertion.id, insertion.text)
      }
      return
    }
    if (event?.type !== 'tool/result') return
  ...
  private tick(): void {
    ...
    // 插话的**探测**是事件驱动的(queueSteer);这里只负责把**没投出去的再试**,
    // 按 steerPollMs 节流(投递本身有 inFlight 防重,不会叠加请求)。
    const now = Date.now()
   ...
   */
  private queueSteer(id: string, text: string): void {
    if (id.length === 0 || text.trim().length === 0) return
    if (this.deps.isForwarded?.(id) === true) return
    if (!this.pendingSteers.has(id)) {
      steerDebug(`steer 入队 session=${this.deps.dshSessionId} id=${id}`
        + ` text=${JSON.stringify(text.slice(0, 40))}`)
    }
    this.pendingSteers.set(id, text)
    this.flushPendingSteers()
  }
  ...
  private flushPendingSteers(): void {
    if (this.pendingSteers.size === 0) return
    for (const [id, text] of [...this.pendingSteers]) {
      if (this.deps.isForwarded?.(id) === true) {
        this.pendingSteers.delete(id)
        continue
      }
      this.steerMessage(id, text)
    }
  }
  ...
  private steerMessage(id: string, text: string): void {
    ...
    const conn = this.conn
    // 连接未就绪(进程未起 / session/new 未回)也留在队列:由 tick 重试。
    // 诊断只在**本轮阻塞的第一次**写一行,否则每 1.2s 刷一条。
    if (conn === undefined || this.acpSessionId === '') {
      if (!this.steerBlockedLogged) {
        this.steerBlockedLogged = true
        steerDebug(`steer 排队等待连接 session=${this.deps.dshSessionId}`
          + ` 原因=${conn === undefined ? 'no-conn' : 'no-acp-session'} 队首=${id}`)
      }
      return
    }
    this.steerBlockedLogged = false
    this.inFlightForwards.add(id)
    const fallback = (): void => {
      this.inFlightForwards.delete(id)
      const current = this.conn
      // 连接已散:不标记、**留在队列里**由 tick 重试(投不出 ≠ 静默消失)。
      if (current === undefined) {
        steerDebug(`steer 回退失败(连接已散,留在队列重试)id=${id}`)
        return
      }
