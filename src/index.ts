...
export function apply(ctx: Context, config: Config): void {
  ...
  // (围栏与 /api 网关行为一致,见 models-route.ts 模块头)。
  ctx.effect(() => registerCodebuddyModelsRoute(ctx, () => settingsOf().command))

  // LLM provider 路由:主代理选择与子代理委派的推理都走 CodeBuddy。
  // /compact 转发与 adapter 共用同一会话映射(acpId → dsh 会话)。
  ...
  // codebuddy 会话的 /compact 转发给 CLI(per-agent 命令覆盖;其它会话回落 dsh);
  // 转发跑在 agent 的 maintenance 相位里,压缩期间消息按原生行为排队。
  mountCompactCommand({
    ctx,
    command: resolved.command,
    prefixArgs: resolved.args,
   ...
   */
  function subagentToolOptions(): Parameters<typeof registerSubagentTool>[1] {
    return {
      provider: 'spawn',
      toolName,
      ...
      description:
        ...
        + `selector for the currently supported ids); omit it to use the plugin-configured default model (\`${settingsOf().model}\`). `
  ...
  function syncTools(): void {
    if (toolCtx === undefined) return
    // 按最新设置重建:工具描述里带"当前默认模型",设置改动要随之更新。
    for (const dispose of disposers) dispose()
    disposers = []
    if (!settingsOf().registerSubagentTools) return
    disposers = [
