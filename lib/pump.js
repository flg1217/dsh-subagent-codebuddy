...
export const DSH_DELEGATION_NOTE = [
    ...
    '- Bash/PowerShell/Edit/Write/Glob/Grep 等 CLI 原生工具不在你的工具列表里(白名单已移除):同类能力一律直接用 dsh_<原工具名>;Read 仍可用——读图片必须走 Read。不要换方式绕过(例如把命令包成脚本执行)。',
...
export const MCP_DELEGATION_NOTE = [
    ...
    '- 一律使用 mcp__dsh__<工具名>(bash、pwsh、read、write、edit、glob、grep、subagent、send_message、job_output……)。',
...
export class TurnPump {
    ...
    async runAttempt() {
        ...
        try {
            ...
            this.deps.rememberConversation(this.acpSessionId, this.deps.sentCount, this.deps.sentLastMessageId, this.deps.systemHash);
    ...
    announceCall(callId, rawName, rawInput) {
        ...
        const callDshName = delegateTarget ?? dshName;
        ...
        const mirrorName = delegateTarget ?? (alias !== undefined ? dshName : `${CLI_MIRROR_TOOL_PREFIX}${dshName}`);
