...
import { blocksToText, bridgeTargetTool, CLI_MIRROR_TOOL_PREFIX, DSH_MCP_SERVER_NAME, listDshBridgeTools, McpDispatchAbortedError, McpDispatchTimeoutError, registerDshMcpServer, registerMcpLoopDispatcher, } from '@flg1217/dsh-mcp';
 ...
 */
export function watchLateMcpResult(ctx, sessionId, callId, toolName, sink, ttlMs = LATE_MCP_RESULT_TTL_MS) {
    const on = ctx.on;
    if (on === undefined)
        return;
    let done = false;
    const off = on('session/event', (...args) => {
        if (done)
            return;
        ...
        off?.();
        sink?.(toolName, blocksToText(Array.isArray(block?.content) ? block.content : []));
    });
 ...
 */
export function replayPresentationMeta(value) {
...
const INTERACTIVE_MCP_TOOLS = new Set(['ask_user_question', 'exit_plan_mode']);
/**
...
export function promptTailFor(bridgeMode) {
...
export class TurnPump {
    ...
    constructor(deps, signal, stepState) {
        ...
        this.disposeLoopDispatcher = registerMcpLoopDispatcher(deps.dshSessionId, (_sessionId, name, input, lateSink) => this.dispatchMcpCall(name, input, lateSink));
        ...
        for (;;) {
            ...
            if (segment !== undefined) {
                ...
                if (this.failed !== undefined) {
                    yield* this.yieldError(this.failed);
                    return;
                }
                yield {
                    type: 'finish',
    ...
    async runAttempt() {
        ...
        // 启动,该回合零工具且完全静默。
        if (this.bridgeMode === 'mcp')
            registerDshMcpServer(this.deps.ctx);
        const argv = [
            this.deps.command,
        ...
        const conn = new AcpConnection(argv, this.deps.cwd, update => this.onUpdate(update), request => this.handleClientRequest(request));
        this.conn = conn;
        conn.onExit(info => {
            if (conn.wasKilled || this.stallTimedOut || this.disposed)
                return;
            // 进程退出沿用旧语义:不可续跑(重试会重复已完成部分),显式中止。
            this.failRun(new Error(`CodeBuddy ACP 进程退出(code ${info.code ?? 'null'}${info.signal !== null ? `,signal ${info.signal}` : ''})${conn.stderrNote()}`), false);
        });
        try {
            ...
            const parentAgent = this.deps.ctx.get('agents')?.get(this.deps.dshSessionId);
            ...
            for (let round = 0; round < 2 && failedBatches.length > 0; round += 1) {
                ...
                for (const batch of pending) {
                    try {
                        await announceDelegateTools((method, params) => conn.request(method, params, this.to.firstMs), this.acpSessionId, batch);
                    }
                    catch {
            ...
            void this.sendPrompt(this.deps.prompt, this.deps.images).then(ok => {
                if (ok && sentAcpId !== '') {
                    this.deps.rememberConversation(sentAcpId, this.deps.sentCount, this.deps.sentLastMessageId, this.deps.systemHash);
                }
            });
            this.armProgress();
        }
    ...
    sendPrompt(prompt, images) {
        ...
        return conn.request('session/prompt', {
            ...
            if (this.attemptToken !== token)
                return;
            if (this.promptError === undefined && this.promptFailure === undefined)
    ...
    failRun(error, retryable = true) {
        if (this.finished || this.disposed)
            return;
        if (this.canRestart(retryable)) {
            this.restart();
            return;
        }
        this.failed = error;
     ...
     */
    appendFailureClosingMessage(error) {
        if (this.hasText)
            return;
        if (this.deps.childSession?.header?.parentSession === undefined)
            return;
        const append = this.deps.session?.append;
        const at = this.deps.turnStep;
        if (append === undefined || at === undefined)
            return;
        try {
    ...
    dispose() {
        ...
        for (const waiter of this.delegateWaiters.values())
            waiter.reject(new Error('CodeBuddy 回合已结束'));
    ...
    async awaitResult(callId, signal) {
        ...
        if (call.outcome === undefined && signal?.aborted === true)
     ...
     *   CLI↔MCP↔loop 三方死锁)。
     */
    ensureReplayTool(name, nonBlocking = false) {
        ...
        try {
            ...
            if ((tools.schemas?.(agent) ?? []).some(schema => schema?.name === name))
                return;
            ...
            if (typeof dispose === 'function')
                this.replayToolDisposers.push(dispose);
        }
        catch (error) {
    ...
    async handleClientRequest(request) {
        if (request.method !== DELEGATE_TOOL_METHOD)
            return undefined;
        ...
        if (toolId.length === 0) {
            const parent = this.deps.ctx.get('agents')?.get(this.deps.dshSessionId);
    ...
    subscribeSessionEvents() {
        ...
        if (ctx.on === undefined)
            return undefined;
        const handler = (...args) => {
            this.onSessionEvent(args[0], args[1]);
        };
    ...
    onSessionEvent(session, event) {
        ...
        if (late !== undefined) {
            ...
            late.sink?.(late.name, blocksToText(Array.isArray(block?.content) ? block.content : []));
            return;
        }
        ...
        if (waiter !== undefined) {
            ...
            waiter.resolve({
                output: blocksToText(Array.isArray(block?.content) ? block.content : []),
                isError: block?.isError === true,
            });
            return;
        }
        ...
        const call = this.calls.get(callId);
        if (call === undefined || call.rawName !== 'delegate_tool')
            return;
    ...
    claimDelegateRequest(callId, toolId, input) {
        ...
        if (request !== undefined) {
            if (pending !== undefined && pending.length === 0)
                this.pendingDelegateRequests.delete(toolId);
     ...
     */
    async dispatchMcpCall(name, input, lateSink) {
        if (this.finished || this.disposed)
            throw new Error('CodeBuddy 回合已结束(模型可能已切换)');
        ...
        return await new Promise((resolve, reject) => {
            ...
            const timer = timeoutMs > 0
                ? setTimeout(() => {
                    ...
                    reject(new McpDispatchTimeoutError(`MCP 调用 ${name} 注入后 ${Math.round(timeoutMs / 1000)}s 内未被 loop 消费`));
                }, timeoutMs)
                : undefined;
            timer?.unref?.();
            this.mcpWaiters.set(callId, {
                resolve: result => { if (timer !== undefined)
                    clearTimeout(timer); resolve(result); },
                reject: error => { if (timer !== undefined)
     ...
     */
    awaitDelegateResult(toolId, input, signal) {
        const args = JSON.stringify(input);
        const queue = this.unclaimedDelegates.get(toolId);
        const entry = queue === undefined ? undefined : takeMatch(queue, args);
        if (queue !== undefined && queue.length === 0)
            this.unclaimedDelegates.delete(toolId);
        if (entry === undefined) {
    ...
    bindDelegateWaiter(callId, resolve, reject, signal) {
        const onAbort = () => {
            if (this.delegateWaiters.get(callId) === waiter)
                this.delegateWaiters.delete(callId);
     ...
     * 保持原有镜像行为不变。
     */
    isSuppressedBridgeCall(rawName) {
        if (this.bridgeMode !== 'mcp')
            return false;
        if (rawName === 'defer_execute_tool')
            return true;
    ...
    announceCall(callId, rawName, rawInput) {
        ...
        const malformedDelegate = rawName === 'delegate_tool'
            && (typeof rawInput['toolId'] !== 'string' || rawInput['toolId'].length === 0);
        ...
        if (!isDelegateCall && /"run_in_background"\s*:\s*true|"background"\s*:\s*true/.test(argsJson))
        ...
        if (delegateTarget === undefined)
            this.ensureReplayTool(mirrorName, nonBlockingReplay);
        const segment = this.openSegment();
        this.closeOpenBlock(segment);
        const index = segment.blockIndex++;
        segment.calls.push(callId);
        ...
        if (delegateTarget !== undefined)
            this.claimDelegateRequest(callId, String(rawInput['toolId']), delegateInput);
    ...
    checkTail() {
        ...
        if (this.promptFailure !== undefined) {
            this.failRun(new Error(`CodeBuddy 中断:${formatFailureLine(this.promptFailure, this.conn?.stderrNote())}`), this.promptFailure.retryable);
            return;
        }
        if (this.promptError !== undefined) {
            const failure = failureOfError(this.promptError);
            this.failRun(new Error(`CodeBuddy ACP 请求失败:${formatFailureLine(failure, this.conn?.stderrNote())}`), failure.retryable);
            return;
        }
        if (!this.promptSettled || this.inFlight > 0 || this.aborted) {
            ...
            if (!this.aborted && this.agentPhaseSeen && this.lastIdlePhaseAt !== undefined
                ...
                if (armed && budget > 0 && silentFor >= budget) {
                    ...
                    if (seg !== undefined && !seg.closed)
                        this.closeSegment(seg, 'settle');
                    this.finishTurn();
                    return;
                }
    ...
    pollInsertions() {
        ...
        try {
            ...
            if (this.lastScannedIndex < 0)
                this.lastScannedIndex = Math.max(0, this.insertionFloorIndex(events));
            // 1) 未 claim 的悬挂插入(next-step):赶在 claim 前投递。
            // 折叠是 O(events) 的全量重建:events 未变化(长度 + 尾 seq 都同)时
            // 复用上轮结果,长会话空闲期不再每 1.2s 全量重折。
            const tail = events.length === 0 ? '' : String(events[events.length - 1]?.seq ?? '');
    ...
    steerMessage(id, text) {
        ...
        void conn.request('session/steer', { sessionId: this.acpSessionId, contentBlocks: [{ type: 'text', text }] }, 20_000).then(result => {
