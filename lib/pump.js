...
export class TurnPump {
    ...
    constructor(deps, signal, stepState) {
        ...
        this.disposeLoopDispatcher = registerMcpLoopDispatcher(deps.dshSessionId, (_sessionId, name, input) => this.dispatchMcpCall(name, input));
     ...
     */
    async dispatchMcpCall(name, input) {
        if (this.finished || this.disposed)
        ...
        return await new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                reject(new McpDispatchTimeoutError(`MCP 调用 ${name} 注入后 ${Math.round(MCP_CALL_TIMEOUT_MS / 1000)}s 内未被 loop 消费`));
            }, MCP_CALL_TIMEOUT_MS);
            timer.unref?.();
