...
function categoryOfRpcCode(code) {
    switch (code) {
 ...
 */
export function parseCodebuddyFailure(raw, rpcCode, outcome) {
    const text = raw?.trim();
    if ((text === undefined || text.length === 0) && rpcCode === undefined && outcome === undefined)
        return undefined;
    let message;
    let display;
    let category;
    ...
    if (text !== undefined && text.length > 0) {
        ...
        if (parsed === undefined) {
            ...
            if (data !== undefined && data !== null && typeof data === 'object' && !Array.isArray(data)) {
                ...
                if (typeof record['subcategory'] === 'string')
                    subcategory = record['subcategory'];
                if (typeof record['reason'] === 'string' && message === undefined)
    ...
    if (category === undefined && bizCode !== undefined)
        category = BIZ_CODE_CATEGORY[bizCode];
...
function displayMessageOf(value) {
    if (value === null || typeof value !== 'object' || Array.isArray(value))
        return undefined;
    const record = value;
    const preferred = firstString(record['zh'], record['zh-Hans'], record['zh-hant'], record['zh-Hant'], record['en'], record['en-US']);
 ...
 */
export function failureOfError(error) {
    if (error instanceof AcpRpcError) {
        ...
        if (failure !== undefined && failure.category !== 'unknown')
            return failure;
    }
 ...
 * @returns 单行文本。
 */
export function formatFailureLine(failure, suffix) {
    const meta = [
