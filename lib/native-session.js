...
export function conversationFilePath(sessionId, cwd, baseDir) {
 ...
 */
export async function messagesToRecords(messages, context, images, system, bridgeMode = 'delegate') {
    const records = [];
    ...
    for (const message of messages) {
        ...
        if (images !== undefined && imageBlocks.length > 0) {
            ...
            for (const block of imageBlocks) {
                try {
                    const stored = await images.readImage(block.attachment);
...
export function sessionMetaRecords(context) {
    const now = Date.now();
    return [
        { id: uuidv7(), timestamp: now, type: 'session-meta', sessionId: context.sessionId, meta: { 'codebuddy.ai/hostKind': 'unopted' } },
    ];
}
/** 写入(覆盖)会话文件;原子写(tmp + rename)。 */
export function writeConversationFile(file, records, context) {
...
/** 追加记录到已有会话文件(缺失轮次补写)。 */
export function appendConversationRecords(file, records) {
    if (records.length === 0)
        return;
    mkdirSync(dirname(file), { recursive: true });
