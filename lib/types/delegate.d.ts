...
export declare const DELEGATE_TOOL_METHOD = "_codebuddy.ai/delegateTool";
/** 委托工具变更通知的方法名(客户端 → CLI)。 */
export declare const DELEGATE_TOOLS_CHANGED_METHOD = "_codebuddy.ai/delegateToolsChanged";
/** 注册给 CodeBuddy 的一个委托工具。 */
...
/** 注册函数面(request = 连接上的 JSON-RPC 请求)。 */
export type DelegateRequestFace = (method: string, params: Record<string, unknown>) => Promise<unknown>;
/** CLI 侧 DelegateTool 的响应形状。 */
export type DelegateToolResult = {
    status: 'success';
    output: string;
} | {
    status: 'error';
 ...
 */
export declare function announceDelegateTools(request: DelegateRequestFace, sessionId: string, tools: readonly DelegateToolSpec[]): Promise<void>;
