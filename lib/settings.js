...
export function registerCodebuddySettings(ctx, config, onSettingsChange) {
    ...
    ctx.inject(['settings'], (settingsCtx) => {
        ...
        settings?.installSection?.(ctx, CODEBUDDY_SETTINGS_NAMESPACE, CodebuddySettingsConfig, { bridgeMode: config.bridgeMode ?? 'mcp' }, {
            setSource: (source) => {
                current = (() => source() ?? {});
            },
    ...
    const sectionOf = () => {
        ...
        const filled = (value) => typeof value === 'string' && value.trim().length > 0 ? value : undefined;
        return {
