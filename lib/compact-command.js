 ...
 * 里的寻址说明)。拿不到时返回 undefined:要么 dsh 没有名册服务,要么该会话的
 * preset 本来就没挂 compaction(那也就不存在压缩风暴)。
 */
function engineForAgent(deps, agent) {
    ...
    if (roster?.serviceFor === undefined) {
        reportOnce(deps.ctx, 'no-roster', `${LOG_TAG} 拿不到 agentPresets.serviceFor:压缩接管未安装,`
            + 'codebuddy 会话可能出现 dsh 侧压缩风暴(需要带 preset 名册的 dsh)');
        return undefined;
    }
    try {
