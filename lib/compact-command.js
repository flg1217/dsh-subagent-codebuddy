...
async function dshCompact(deps, invocation) {
    ...
    try {
        ...
        // 必须走 cause 链:dsh 的 manual 压缩会把真实失败**包装**成
        // `ManualCompactionError('summary', 'manual compaction could not produce a
        // smaller summary', { cause })`(compaction-basic/src/region.ts:274),
        // 只取 `error.message` 会把真因吞掉 —— 实测 2026-09-22 用户报障时回执只有
        // "could not produce a smaller summary",而会话里 `compaction/end.error`
        // 记的才是真因(`upstream stream closed before a terminal event`)。
        return { kind: 'error', text: `Compaction failed: ${errorChain(error)}` };
    }
}
/**
 ...
 */
function engineForAgent(deps, agent) {
    if (agent === undefined)
        return undefined;
    const roster = serviceOf(deps.ctx, 'agentPresets');
    if (roster?.serviceFor === undefined) {
        const host = serviceOf(deps.ctx, 'compaction');
        if (host !== undefined)
            return host;
    ...
    try {
        const engine = roster.serviceFor(agent, 'compaction');
        if (engine === undefined) {
...
function reportOnce(ctx, key, message) {
    if (reportedReasons.has(key))
        return;
...
export function resetTakeoverStateForTests() {
...
export async function handleCompactCommand(deps, invocation) {
    ...
    // 的注释同源。
    if (!isCodebuddyOwned(deps, invocation.agent))
