    ...
    function CodebuddyCard(props) {
      ...
      const loadConfig = react.useCallback(async () => {
        try {
          ...
          const ns = resp?.result?.value?.namespaces?.find?.((n) => n.ns === 'codebuddy')
          const v = ns?.value ?? {}
          const model = v.model ?? ''
      ...
      const loadModels = react.useCallback(async () => {
        ...
        try {
          const resp = await connection.api.llm.discoverModels({ settingsNs: 'codebuddy', provider: 'models' })
          const found = resp?.result?.ok ? (resp.result.value?.models ?? []) : []
          setModels(found.map((m) => m.name ?? m.id).filter(Boolean))
      ...
      const writeModel = react.useCallback(async (value) => {
        try {
          ...
          await connection.api.settings.mutate({ ns: 'codebuddy', ops: [op] })
