...
export class CodebuddyLlmAdapter extends LlmAdapter {
     ...
     */
    async startTurn(options, sessionId, childSession, turnStep) {
    ...
    async buildNativeSeed(options, cwd) {
        ...
        try {
            ...
            options.system, this.options.bridgeMode ?? 'delegate');
            if (records.length === 0)
                return undefined;
    ...
    async *oneShotOnce(options, state) {
        ...
        try {
            ...
            if (this.options.bridgeMode === 'mcp')
