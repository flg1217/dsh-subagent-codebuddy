...
export class CodebuddyLlmAdapter extends LlmAdapter {
    ...
    async *oneShotOnce(options, state) {
        ...
        try {
            ...
            const conn = new AcpConnection([command, ...prefixArgs, '--acp', '--model', model, ...effortArgs, '--dangerously-skip-permissions', ...cliToolPolicyArgs(), ...this.options.extraArgs], cwd, onUpdate);
            let exitError;
            conn.onExit(info => {
