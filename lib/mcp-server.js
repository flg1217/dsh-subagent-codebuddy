            ...
            if (dispatcher !== undefined) {
                ...
                try {
                    ...
                    if (clientGoneAt !== undefined) {
                        console.error(`[codebuddy-bridge] MCP 调用在客户端断连后才完成:session=${sessionId}`
                            + ` tool=${name} 客户端等待=${clientGoneAt - startedAt}ms(≈CLI 侧超时 T_client)`
                            + ` 工具总耗时=${Date.now() - startedAt}ms`
                    ...
                    if (error instanceof McpDispatchTimeoutError) {
                        ...
                        console.error(`[codebuddy-bridge] MCP 转发兜底超时(不回落重执):session=${sessionId}`
                            + ` tool=${name} clientGone=${clientGoneAt !== undefined} err=${error.message}`);
                        return toolError(`${error.message};为避免同一副作用重复执行,未回落直执——`
                            + '请先在会话时间线确认该调用是否已执行,不要盲目重跑同一命令。');
                    }
                    // 其余转发失败(回合收尾竞态等):泵不在、工具尚未执行,回落直接执行;
                    // 但不静默——这条路径意味着 dsh 侧不会出现对应卡片,留现场日志便于定位。
                    console.error(`[codebuddy-bridge] MCP 转发未成,回落直执:session=${sessionId} tool=${name}`
