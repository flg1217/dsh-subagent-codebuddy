 ...
 *
 * 只在客户端确实断连时才投递:那时 CLI 一定没收到响应,所以这不是重复上报。
 * @param ctx - 插件上下文(取 agents 服务)。
 * @param sessionId - dsh 会话 id。
 * @param toolName - 被调用的 dsh 工具名。
 * @param text - 工具结果正文。
 */
function deliverAfterClientGone(ctx, sessionId, toolName, text) {
    ...
    const message = createUserMessage({
        content: [{
                type: 'text',
                text: `[dsh] 你先前调用 dsh 工具 \`${toolName}\` 时,客户端在结果返回前就放弃了这次调用`
        ...
        source: {
            ...
            summary: boundContextSummary(`dsh 工具 ${toolName} 在客户端放弃后才返回,结果已补投`),
        },
    });
    try {
...
function bridgeLog(line) {
    ...
    try {
        ...
        appendFileSync(join(homedir(), '.dsh', 'codebuddy', 'mcp-bridge.log'), `${new Date().toISOString()} ${line}\n`);
    }
    catch { /* 诊断不影响主流程 */ }
}
/**
 ...
 */
export function sweepStaleMcpConfigs(dir = mcpConfigDir(), maxAgeMs = 60 * 60 * 1000) {
    let removed = 0;
    try {
        const now = Date.now();
 ...
 */
export function writeDshMcpConfigFile(dshSessionId) {
 ...
 */
export function mcpConfigArgs(bridgeMode, dshSessionId) {
    ...
    if (bridgeMode !== 'mcp' || dshSessionId === undefined)
        return [];
    ...
    switch (msg.method) {
        ...
        case 'tools/call': {
            ...
            if (dispatcher !== undefined) {
                try {
                    const result = await dispatcher(sessionId, name, input);
            ...
            if (clientGoneAt !== undefined) {
                ...
                console.error(line);
                bridgeLog(line);
                deliverAfterClientGone(ctx, sessionId, name, result.status === 'error' ? result.error.message : result.output);
            }
