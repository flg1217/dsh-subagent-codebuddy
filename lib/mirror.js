...
export function agentIdFromOutput(text) {
...
export class RecordTranslator {
    ...
    todos = new TodoListState();
    /** read_image 别名调用的 meta 路径(callId → path,结果落地时写 meta)。 */
    imageReadPaths = new Map();
    constructor(append, model, images) {
        this.append = append;
        this.model = model;
        this.images = images;
    }
    ...
    async apply(record) {
        switch (record.type) {
            ...
            case 'function_call': {
                ...
                if (alias !== undefined)
                    this.imageReadPaths.set(record.callId, alias.path);
                ...
                if (typeof event?.seq === 'number')
                    this.callSeqs.set(record.callId, event.seq);
                ...
                if (kind !== undefined) {
                    let parsed = {};
                    try {
                        parsed = (typeof record.arguments === 'string' ? JSON.parse(record.arguments) : record.arguments);
                    }
                    catch { /* 参数非法则跳过 */ }
                    if (kind === 'taskupdate') {
            ...
            case 'function_call_result': {
                ...
                let content = [{ type: 'text', text: text.slice(0, 4000) }];
                if (!isError) {
                    const face = this.images === undefined
                        ? undefined
                        : { saveImage: (input) => this.images.saveImage(input.data, input.mediaType) };
...
export class SubagentMirror {
    ...
    /** 开始镜像:建影子会话并进入跟随。 */
    start(agent) {
        if (this.shadow !== undefined || this.finished)
            return;
        try {
        ...
        this.translator = new RecordTranslator((type, data, opts) => shadow.append(type, data, opts), 'codebuddy', this.deps.attachments);
    ...
    locateFile() {
        ...
        // readdir 是目录序而非时间序:回退取"最新"必须按 mtime 排序,否则并行
        // 多个委派时(提示词前缀都没匹配上)会镜像到错误的子代理转录。
        const candidates = entries
            .map(name => {
            const path = join(this.subagentsDir(), name);
            try {
            ...
            .filter((entry) => entry !== undefined && entry.mtimeMs >= this.startedAt - 5_000)
        ...
        for (const path of candidates) {
            try {
                ...
                if (first !== undefined && prefix.length > 0 && first.includes(JSON.stringify(prefix).slice(1, -1)))
                    return path;
            }
            catch { /* 读失败跳过 */ }
        }
    ...
    async syncOnceUnsafe() {
        if (this.translator === undefined || this.finished)
            return;
        ...
        while (this.offset < lines.length) {
            let record;
            try {
                record = JSON.parse(lines[this.offset]);
            }
            catch {
                return; // 半行(正在追加):下轮再读
            }
            ...
            if (record.type === 'session-meta' || record.type === 'file-history-snapshot' || record.type === 'ai-title')
                continue;
